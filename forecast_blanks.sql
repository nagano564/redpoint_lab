...
CREATE OR REPLACE VIEW combined_table_honda AS (
    SELECT TO_DATE(date) AS date, revenue, NULL as FORECAST, NULL as LOWER_BOUND, NULL AS UPPER_BOUND FROM ???
    UNION 
    SELECT TO_DATE(TS) AS date, NULL as revenue, FORECAST, LOWER_BOUND, UPPER_BOUND from ???
    ORDER BY date DESC
);
