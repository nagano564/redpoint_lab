...
def sentiment_analysis():
    with st.container():
        ...
        if entered_text:
            ...
            cortex_response = session.sql(f"select AUTHOR_ID, market_insights, snowflake.cortex.sentiment(market_insights) as sentiment_score from market_trends;").to_pandas()
            st.caption("Score is between -1 and 1; -1 = Most negative, 1 = Positive, 0 = Neutral")  
            st.write(cortex_response)

            response_for_graph = session.sql(f"SELECT AUTHOR_ID, AVG(snowflake.cortex.sentiment(market_insights)) AS avg_sentiment FROM market_trends GROUP BY AUTHOR_ID;").to_pandas()  
